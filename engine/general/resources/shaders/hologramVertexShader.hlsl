...
vertexOutput main(vertexInput input)
{
    ...
    //output.position = mul(mul(float4(vertexDistortion(input.position, input.normal), 1), input.instance), VP);
    //output.position = float4(vertexDistortion(input.position, input.normal), 1);
