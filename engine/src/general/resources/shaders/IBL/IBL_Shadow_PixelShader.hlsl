...
float3 CalculateLightPBR( float3 normal, float3 surfaceNormal, float3 lightDirection, float3 viewDirection, float3 lightColor, float solidAngle, float lightRadius, float intensity, float roughness, float metalness, float3 albedo);
...
float4 CalculatePointLightShadowFactorDebug(float3 dirToLight, float3 worldPosition, float3 normal, PointLight light, int index);
//-------------------------------------------------------------------------------------------------------
...
float3 CalculatePointLightPBR(float3 normal, float3 macroNormal, float3 position, PointLight g_pointLight, float roughness, float metalness, float3 albedo)
{
    ...
    float3 viewDirection = normalize(g_cameraPosition.xyz - position);
