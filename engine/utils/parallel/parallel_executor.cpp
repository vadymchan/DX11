...
const uint32_t Engine::ParallelExecutor::MAX_THREADS = std::max(1u, std::thread::hardware_concurrency());
const uint32_t Engine::ParallelExecutor::HALF_THREADS = std::max(1u, std::thread::hardware_concurrency() / 2);

Engine::ParallelExecutor::ParallelExecutor(uint32_t numThreads)
{
	m_isLooping = true;
...
Engine::ParallelExecutor::~ParallelExecutor()
{
	wait();
	m_isLooping = false;
	awake();
...
void Engine::ParallelExecutor::wait()
{
	if (!isWorking()) return;
...
void Engine::ParallelExecutor::execute(const Func& func, uint32_t numTasks, uint32_t tasksPerBatch)
{
...
void Engine::ParallelExecutor::executeAsync(const Func& func, uint32_t numTasks, uint32_t tasksPerBatch)
{
	if (numTasks == 0) return;
...
void Engine::ParallelExecutor::awake()
{
	m_workCV.notify_all();
}

void Engine::ParallelExecutor::workLoop(uint32_t threadIndex)
{
	while (true)
	{
		{
